...
vec3 ray_color(Ray ray) {
  ...
  Sphere[] sphere_list = Sphere[](
    Sphere(vec3(0.0, 0.1 * sin(u_time*2.0), -1.0), 0.5),
  ...
  for(ray_i = 0; ray_i < depth; ray_i++) {
    ...
    for (int i=0; i<sphere_list.length(); i++) {
      bool intersect = ray_sphere_intersection(ray_next, Interval(0.001, closest), sphere_list[i], rec_temp);
    ...
    vec3 direction = record.normal + random_unit_vector(vec2(gl_FragCoord.xy));
    ray_next = Ray(record.pos, direction);
